...
const Categories = () => {
  ...
  const fetchCategories = async () => {
    try {
      setLoading(true);
      const res = await axiosInstance.get('/catalog/admin/categories/');
      setCategories(res.data.results || res.data); // Handle pagination if present
    } catch (error) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingCategory) {
        await axiosInstance.put(`/catalog/admin/categories/${editingCategory.id}/`, formData);
      } else {
        await axiosInstance.post('/catalog/admin/categories/', formData);
      }
      setModalOpen(false);
      fetchCategories();
    } catch (error) {
      console.error("Failed to save category", error);
      alert("Failed to save category. Please check your input.");
    }
  };
  ...
  const handleDelete = async (id) => {
    if (window.confirm("Are you sure you want to delete this category?")) {
      try {
        await axiosInstance.delete(`/catalog/admin/categories/${id}/`);
        fetchCategories();
      } catch (error) {
            ...
            {loading ? (
              ...
              categories.map(cat => (
                <tr key={cat.id} className="hover:bg-neutral-50 dark:hover:bg-neutral-800/50">
                  ...
                  <td className="px-6 py-3 font-medium text-neutral-900 dark:text-neutral-100">{cat.name}</td>
                  <td className="px-6 py-3 text-neutral-500">{cat.slug}</td>
                  <td className="px-6 py-3 flex items-center justify-end gap-3">
                    <button onClick={() => openModal(cat)} className="text-neutral-400 hover:text-neutral-900 dark:hover:text-white transition-colors">
                      <Edit2 size={16} />
                    </button>
                    <button onClick={() => handleDelete(cat.id)} className="text-neutral-400 hover:text-red-600 transition-colors">
                      <Trash2 size={16} />
                    </button>
                  </td>
                </tr>
